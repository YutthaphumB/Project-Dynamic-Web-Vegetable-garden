...
<body>
    <div class="view" style="
      ...
      ">
        ...
        <br>
        <h2 align="center">แก้ไขข้อมูลผัก</h2>
        <br>

        <?php
        include "connection.php";
        try {
            $sql = "SELECT * FROM vegetable_table  WHERE key_id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(1, $_GET["id"]);
            $stmt->execute();
            $data = $stmt->fetch();

            $sql2 = "SELECT * FROM card_table  WHERE id_card=?";
            $stmt2 = $conn->prepare($sql2);
            $stmt2->bindParam(1, $data['id_card']);
            $stmt2->execute();
            $data2 = $stmt2->fetch();
                ...
                <div class="col-9">
                    <form method="POST" enctype="multipart/form-data" style="background-color: #F8F8FF;">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" placeholder="กรอก id" name="key_id" value="<?php echo $data['key_id'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">ชื่อ</label>
                            <input type="text" class="form-control" placeholder="กรอก ชื่อ" name="name" value="<?php echo $data['name'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">ชื่อพฤกษศาสตร์</label>
                            <input type="text" class="form-control" placeholder="กรอก ชื่อพฤกษศาสตร์" name="bota_name" value="<?php echo $data['bota_name'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">วงศ์</label>
                            <input type="text" class="form-control" placeholder="กรอก วงศ์" name="family" value="<?php echo $data['family'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">ประวัติ</label>
                            <input type="text" class="form-control" placeholder="กรอก ประวัติ" name="history" value="<?php echo $data['history'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">ลักษณะโดยทั่วไป</label>
                            <input type="text" class="form-control" placeholder="กรอก ลักษณะโดยทั่วไป" name="nature" value="<?php echo $data['nature'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">การปลูก</label>
                            <input type="text" class="form-control" placeholder="กรอก การปลูก" name="plan" value="<?php echo $data['plan'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">ประโยชน์</label>
                            <input type="text" class="form-control" placeholder="กรอก ประโยชน์" name="benefit" value="<?php echo $data['benefit'] ?>">
                        </div><br>
                        <div class="form-group">
                            <label for="formGroupExampleInput2">รหัสการ์ด</label>
                            <input type="text" class="form-control" placeholder="กรอก รหัสการ์ด" name="id_card" value="<?php echo $data['id_card'] ?>">
                        </div><br>
                        <div class="form-group">
...
if (isset($_POST['submit'])) {
    ...
    if (empty($key_id) or empty($name) or empty($bota_name) or empty($family) or empty($history) or empty($id_card) or empty($nature) or empty($plan) or empty($benefit)) {
        ...
        try {
            $stmt = $conn->prepare("UPDATE vegetable_table SET key_id = ?,name = ?,bota_name = ?,family = ?,history = ?,nature = ?,plan = ?,benefit = ?,id_card = ?,img1 = ?,img2 = ? WHERE key_id = ?");
            ...
            $stmt->bindParam(12, $key_id);
            $stmt->execute();

            $stmt2 = $conn->prepare("UPDATE  card_table set id_card = ?,bota_name = ?,family = ?,img = ? WHERE id_card = ?");
            $stmt2->bindParam(1, $id_card);
            $stmt2->bindParam(2, $bota_name);
            $stmt2->bindParam(3, $family);
            $stmt2->bindParam(4, $fileImgc);
            $stmt2->bindParam(5, $id_card);

            $stmt2->execute();
            phpAlert("แก้ไขเรียบร้อย");
        } catch (PDOException $e) {
