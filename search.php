    ...
    <div class="container">
      <div class="row justify-content-md-center">
        ...
        if (isset($_POST['submit'])) {
          ...
          $sql2 = $conn->prepare("SELECT * FROM vegetable_table WHERE name like '%$keyword%'");
          $sql2->execute();
          $publisher = $sql2->fetchAll(PDO::FETCH_ASSOC);
        foreach ($publisher as $sql2){
          $sql = $conn->prepare("SELECT * FROM card_table WHERE id_card = $sql2[id_card]");
          $sql->execute();
          $data = $sql->fetch();
        ?>
            ...
            <div class="col-3">
              <div class="card">
                ...
                <div class="card-body">
                  <p class="card-text">
                    <strong>ชื่อพฤกษศาสตร์ : </strong> <?php echo $data["bota_name"] ?> <br />
                    <strong>วงศ์ : </strong> <?php echo $data["family"] ?>
                  </p>
                </div>
                <div class="card-footer text-muted text-center">
                  <button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#v<?php echo $data["id_card"] ?>">รายละเอียดเพิ่มเติม</button>
                </div>
              </div>
            </div>
...
if (isset($_POST['submit'])) {
  include 'connection.php';
  $keyword = $_POST['search'];
  $sql = $conn->prepare("SELECT * FROM vegetable_table WHERE name like '%$keyword%'");
  $sql->execute();
